...
'''
Run the below files to generate the data, data_val, order_train as well as order_test
'''
    ...
    #path = "data"
    path = "C:/Users/Germayne/Desktop/imba-master/data"
    ...
    #markov_chain_products = pd.read_csv('data/transition-probabilities-by-product.csv')
    ############### train
    ...
    order_train = pd.merge(order_train, order_streaks, how = 'left', on =['user_id',  'product_id'])
    #order_train = pd.merge(order_train, markov_chain_products, how = 'left', on =['product_id'])
    ##############
    ...
    order_test = pd.merge(order_test, order_streaks, how = 'left', on =['user_id',  'product_id'])
    #order_test = pd.merge(order_test, markov_chain_products, how = 'left', on =['product_id'])
    ...
    del(aisles,departments,order_prior,order_stat,prod_stat,prod_usr,prod_usr_reordered,product_periods)
    del(orders_products)
    del(order_train)
    gc.collect()
    ...
    # train
    #np.random.seed(12357) # let us set some seed 
    ...
    result.to_pickle('data/prediction_lgbm_1759.pkl')
    result.to_csv('data/prediction_lgbm_1759.csv', index = False)
